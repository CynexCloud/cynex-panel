...
registerPermission('cynex.admin.addons.view');
registerPermission('cynex.admin.addons.toggle');
registerPermission('cynex.admin.addons.reload');
registerPermission('cynex.admin.addons.store');
registerPermission('cynex.admin.addons.install');
registerPermission('cynex.admin.addons.settings' as Permission);
registerPermission('cynex.admin.addons.commands' as Permission);

const ADDONS_REPO_OWNER = 'CynexCloud';
...
const addonsModule: Module = {
  ...
  router: () => {
    ...
    router.get(
      '/admin/addons',
      isAuthenticated(true, 'cynex.admin.addons.view'),
    ...
    router.get(
      '/admin/addons/store',
      isAuthenticated(true, 'cynex.admin.addons.store'),
      async (req: Request, res: Response) => {
        try {
          const userId = req.session?.user?.id;
          const user = await prisma.users.findUnique({ where: { id: userId } });
      ...
      async (_req: Request, res: Response) => {
        try {
          const contentsRes = await fetch(`${GITHUB_API_BASE}/contents`, {
            headers: { 'Accept': 'application/vnd.github+json', 'User-Agent': 'cynex-panel' },
          });
            ...
            folders.map(async (folder: any) => {
              try {
                const infoRes = await fetch(`${ADDONS_RAW_BASE}/${folder.name}/info.json`, {
                  headers: { 'User-Agent': 'cynex-panel' },
                });
    ...
    router.post(
      ...
      isAuthenticated(true, 'cynex.admin.addons.install'),
      async (req: Request, res: Response) => {
        const { slug } = req.body;
    ...
    router.post(
      ...
      isAuthenticated(true, 'cynex.admin.addons.toggle'),
      async (req: Request, res: Response) => {
        try {
          const slug = getParamAsString(req.params.slug);
    ...
    router.post(
      '/admin/addons/reload',
      isAuthenticated(true, 'cynex.admin.addons.reload'),
      async (req: Request, res: Response) => {
        try {
    ...
    router.post(
      ...
      isAuthenticated(true, 'cynex.admin.addons.settings'),
      async (req: Request, res: Response) => {
        try {
          const slug = getParamAsString(req.params.slug);
          const addon = await prisma.addon.findUnique({ where: { slug } });
    ...
    router.post(
      ...
      isAuthenticated(true, 'cynex.admin.addons.commands'),
      async (req: Request, res: Response) => {
        try {
          const slug = getParamAsString(req.params.slug);
