# CynexPanel - Feature Guide

This is a reference for everything in the Cynex panel. It covers the admin side, the user side, the addon system, and the API.
...
Nodes are the machines that run game servers. Each node runs the Cynex daemon, which the panel communicates with.
...
The marketplace tab lists all community addons from the `CynexCloud/addons` registry. From here you can install addons with one click - the panel clones the repository, runs `pnpm install` and `pnpm run build`, and streams the output back to the browser.
...
Available permission scopes: `cynex.api.servers.*`, `cynex.api.users.*`, `cynex.api.nodes.*`, `cynex.api.settings.*`.
