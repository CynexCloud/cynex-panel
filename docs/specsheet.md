...
| `GET` | `/admin/cynex-cloud` | 👑 | Cynex Cloud settings page |
| `POST` | `/admin/cynex-cloud` | 👑 | Update Cynex Cloud settings |
...
| `GET` | `/api/v1/users` | `cynex.api.users.read` | List users | - |
| `GET` | `/api/v1/users/:id` | `cynex.api.users.read` | Get user by ID | - |
| `POST` | `/api/v1/users` | `cynex.api.users.create` | Create user | `{ email, username, password, isAdmin?, description? }` |
| `PATCH` | `/api/v1/users/:id` | `cynex.api.users.update` | Update user | `{ email?, username?, password?, isAdmin?, description? }` |
| `DELETE` | `/api/v1/users/:id` | `cynex.api.users.delete` | Delete user | - |
| `GET` | `/api/v1/servers` | `cynex.api.servers.read` | List servers | - |
| `GET` | `/api/v1/servers/:id` | `cynex.api.servers.read` | Get server by ID | - |
| `POST` | `/api/v1/servers` | `cynex.api.servers.create` | Create server | `{ name, nodeId, imageId, ownerId, ... }` |
| `PATCH` | `/api/v1/servers/:id` | `cynex.api.servers.update` | Update server | Various fields |
| `POST` | `/api/v1/servers/:id/suspend` | `cynex.api.servers.update` | Suspend server | - |
| `POST` | `/api/v1/servers/:id/unsuspend` | `cynex.api.servers.update` | Unsuspend server | - |
| `DELETE` | `/api/v1/servers/:id` | `cynex.api.servers.delete` | Delete server | - |
| `GET` | `/api/v1/nodes` | `cynex.api.nodes.read` | List nodes | - |
| `GET` | `/api/v1/nodes/:id` | `cynex.api.nodes.read` | Get node by ID | - |
| `POST` | `/api/v1/nodes` | `cynex.api.nodes.create` | Create node | `{ name, address, port, ram, cpu, disk, key, sftpPort? }` |
| `PATCH` | `/api/v1/nodes/:id` | `cynex.api.nodes.update` | Update node | Various fields |
| `DELETE` | `/api/v1/nodes/:id` | `cynex.api.nodes.delete` | Delete node | - |
| `GET` | `/api/v1/settings` | `cynex.api.settings.read` | Get panel settings | - |
| `PATCH` | `/api/v1/settings` | `cynex.api.settings.update` | Update panel settings | `{ title?, description?, logo?, favicon?, theme?, language? }` |
...
2. **Basic Auth** - `Authorization: Basic <base64(Cynex:<key>)>`
3. **HMAC-SHA256** - `X-Cynex-Timestamp`, `X-Cynex-Signature`, `X-Cynex-Nonce` headers
...
Panel                                    Daemon
  ...
  │     X-Cynex-Timestamp: <ts>          │
  │     X-Cynex-Signature: <hex>         │
  │     X-Cynex-Nonce: <nonce>           │
  │──────────────────────────────────────▶  │
...
X-Cynex-Timestamp: 1718476800
X-Cynex-Signature: a1b2c3d4e5f6...
X-Cynex-Nonce: 8f14e45fceea167a5a36dedd4bea2543
...
| Header | Description |
|--------|-------------|
| `X-Cynex-Timestamp` | Unix epoch seconds |
| `X-Cynex-Signature` | HMAC-SHA256 hex digest |
| `X-Cynex-Nonce` | Random nonce (16 bytes hex) |
